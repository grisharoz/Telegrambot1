...
bot=telebot.TeleBot('6960944465:AAHZu2fX4ZwdPnb8VpxrSogcVZrGruRR9oQ')
currency=CurrencyConverter()
amount=0
@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(message.chat.id,"Привет, тебя приветствует бот-конвертатор! Введи сумму:")
...
def summa(message):
    ...
    if amount>0:
     ...
     bot.send_message(message.chat.id,'Выберите пару валют:',reply_markup=markup)
        ...
        bot.send_message(message.chat.id, 'Число должно быть больше чем 0. Введите сумму:')
        bot.register_next_step_handler(message, summa)
